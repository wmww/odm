...
pub fn bundle(snapshot: &ProjectSnapshot) -> Result<Bundle, String> {
    ...
    js.push_str("(() => {\nconst B = (globalThis.__odmBundle = { modules: new Map(), doohickeys: new Map(), versions: {} });\n");
...
mod tests {
    ...
    #[test]
    fn doohickey_bad_import_is_an_export_error() {
        let snap = snapshot(&[(
            "root.js",
            "//! odm unstable\nimport x from './other.js';\nexport default () => null;\n",
        )]);
        let err = bundle(&snap).unwrap_err();
        assert!(err.contains("may only import 'three' and 'odm'"), "{err}");
    }
