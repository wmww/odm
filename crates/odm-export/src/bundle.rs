...
pub struct Bundle {
    pub js: String,
    /// path → transform error, for parts that could not be bundled.
...
/// manifests, plus every part in the snapshot.
pub fn bundle(snapshot: &ProjectSnapshot) -> Result<Bundle, String> {
    ...
    // Parts. Files with pragma errors ship no factory — the scheduler
    ...
    js.push_str("// factory-wrapped framework modules + parts. Consumed by runtime.js.\n");
    js.push_str("(() => {\nconst B = (globalThis.__odmBundle = { modules: new Map(), parts: new Map(), broken: new Map(), versions: {} });\n");
    js.push_str("B.module = (id, deps, fac) => B.modules.set(id, { deps, fac, ns: null, busy: false });\n");
    js.push_str("B.part = (path, api, deps, fac) => B.parts.set(path, { api, deps, fac });\n");
    ...
    for (path, api, e) in &dh {
        let _ = path;
        write_entry(&mut js, "B.part", Some(api.name()), e);
    }
...
mod tests {
    ...
    /// op added on one side and not the other is a part that builds
    ...
    #[test]
    fn bundles_framework_and_parts() {
        let snap = snapshot(&[(
            "root.js",
        ...
        assert!(b.js.contains(r#"B.part("root.js", "unstable""#));
