...
Full reference for the part API. The short version is
`../prompts/js.md`; these files add every function, default, and edge
case.

| Topic | Covers |
| --- | --- |
| [parts.md](parts.md) | The file model, `build(ctx)`, return values, sandbox, console |
...
- **`console`** — `log`/`info`/`debug`/`warn`/`error`, captured into
  build results ([parts.md](parts.md)).
