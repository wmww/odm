...
Valid values are `unstable` and, once cut, `1`, `2`, … (`//! ODM API 1`).
The API version is its own number, separate from the engine's: it moves
only on breaking changes, so a file never needs touching just because
the engine was upgraded. The pragma is
...
`unstable`; once API 1 exists it becomes an error (defaulting to "latest"
...
Cutting API N freezes *behavior*, not code:

- **Semantics, not bytes.** An API N file keeps building with the same
...
**conformance suite** (`tests/conformance/api-N/`) plus its docs snapshot —
...
- The live docs tree (`docs/api/`, served by `odm docs`) tracks
  unstable. A cut copies it to `docs/api-N/`, frozen (typo fixes ok);
  `odm docs --api N` reads the snapshot.
- Each cut writes `docs/changes/api-N.md`: mechanical before → after
  bullets for every breaking change since API N-1.
...
from theory: API 1 happens when the API stops moving under real project
load.

## Version-cut checklist (cutting API N)

1. Add `ApiVersion::V(N)` (pragma name `N`) to
   `crates/odm-build/src/version.rs::SUPPORTED`
   and give it a manifest under `framework/versions/api-N/` (starts as the
   then-current unstable surface; bare `'odm'`/`'three'` resolution per
   version lives in `crates/odm-js/src/snapshot.rs`).
2. Copy `tests/conformance/unstable/` → `tests/conformance/api-N/` and
...
3. Copy the live docs (`docs/api/` + relevant top-level files) →
   `docs/api-N/`; wire `odm docs --api N` to it; teach the doctest runner
   to run that snapshot's examples under API N.
4. Write `docs/changes/api-N.md` (empty-but-present for API 1).
5. For API 1 only: flip missing-pragma from "unstable" to an error
...
6. Update `docs/prompts/` to tell agents to stamp new files with the
   ...
   repo).
7. From now on, run the API N suite forever; behavior-affecting core
   changes need API N shims (`framework/versions/api-N/`) instead of API N
   breakage.
