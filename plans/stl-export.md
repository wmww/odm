...
- **Union everything.** All solids in the scene are boolean-unioned in
  world space, so the file is always one valid manifold. Overlaps fuse;
  disjoint parts stay separate bodies.
- **`units` in `odm.toml`**; export converts to mm (what slicers assume).
  No scale field anywhere.
...
- Existing projects have no key and read as mm; the engine does not
  write it in (its only odm.toml write stays `engine`).
- File ▸ New Project writes `units = "mm"` explicitly; add a units
  dropdown to the dialog (default mm).
...
- `odm-engine/src/stl.rs` (or `scene.rs` neighbour): `export_stl(store,
  kernel, root: &Node, to_mm: f64, cancel) -> Result<StlReport>`:
  1. `collect_meshes` from the root → operands; none → error "nothing
     to export (the scene has no solids)".
  2. One operand with identity-free fast path is not worth it: always
     `kernel.boolean(Union, …)` with the unit scale folded into each
     operand's transform (result is content-addressed, so a re-export
     is a cache hit).
  3. Read the resulting `Mesh`, write **binary STL**: 80-byte header
...
- `StlReport { path, size_mm: [f64;3], volume_mm3, tris, bodies,
  warnings }`. Warnings: `bodies > 1` ("N separate bodies — they will
...
- Writer unit tests: byte-exact golden for a cube (header, count,
  ...
  unit scaling (`m` project cube of 0.02 → 20 mm).
